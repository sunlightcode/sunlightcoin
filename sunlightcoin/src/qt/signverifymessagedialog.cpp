    ...
    model(0)
{
    ...
    ui->signatureOut_SM->setFont(GUIUtil::sunlightcoinAddressFont());
    ui->signatureIn_VM->setFont(GUIUtil::sunlightcoinAddressFont());
}
...
void SignVerifyMessageDialog::on_signMessageButton_SM_clicked()
{
    ...
    CSunlightcoinAddress addr(ui->addressIn_SM->text().toStdString());
    if (!addr.IsValid())
    {
...
{
    CSunlightcoinAddress addr(ui->addressIn_VM->text().toStdString());
    if (!addr.IsValid())
    {
    ...
    if (!(CSunlightcoinAddress(pubkey.GetID()) == addr))
    {
        ui->statusLabel_VM->setStyleSheet("QLabel { color: red; }");

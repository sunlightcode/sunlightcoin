...
*Setup instructions for a gitian build of Sunlightcoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Sunlightcoin
...
- [Building Sunlightcoin](#building-sunlightcoin)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-sunlightcoin-host/README.md).
...
Clone the git repositories for sunlightcoin and gitian and then checkout the sunlightcoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/sunlightcoin/sunlightcoin
cd sunlightcoin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the sunlightcoin repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building Sunlightcoin
----------------

To build Sunlightcoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the sunlightcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/sunlightcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/sunlightcoin/sunlightcoin
...
For example:
```bash
URL=https://github.com/laanwj/sunlightcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit sunlightcoin=${COMMIT} --url sunlightcoin=${URL} ../sunlightcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit sunlightcoin=${COMMIT} --url sunlightcoin=${URL} ../sunlightcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit sunlightcoin=${COMMIT} --url sunlightcoin=${URL} ../sunlightcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/sunlightcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/sunlightcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/sunlightcoin-build.assert
```
...
[sunlightcoin/gitian.sigs](https://github.com/sunlightcoin/gitian.sigs/) repository, or if that's not possible create a pull
